...
def main() -> None:
    parser = argparse.ArgumentParser(description="Text input -> remote local model -> Serial ACT.")
    parser.add_argument("--config", default="config.yaml")
    parser.add_argument("--backend", choices=["ollama", "openai_compatible", "rule_based"], help="Override local backend")
    ...
    backend_name = args.backend or config.get("ai", {}).get("backend", "ollama")
    if backend_name not in {"ollama", "openai_compatible", "rule_based"}:
        print(f"config.yaml ai.backend is '{backend_name}', using 'ollama' for local-remote mode.")
        backend_name = "ollama"

    backend = create_backend(config, map_data, backend_name)
    ...
    try:
        print(f"Local remote mode ({backend_name}). Type Chinese text, or 'quit' to exit.")
        while True:
            text = input("\n你> ").strip()
            if not text:
                continue
            ...
            packet = builder.act(decision, mode="LOCAL")
