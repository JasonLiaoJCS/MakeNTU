...
def capture_latest(camera):
    ...
    image_path = VISION_DIR / datetime.now().strftime("photo_%Y%m%d_%H%M%S.jpg")

    ok = cv2.imwrite(str(image_path), frame, [cv2.IMWRITE_JPEG_QUALITY, 75])
    if not ok:
        print("照片儲存失敗")
        return None
